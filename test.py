...
def test_model(
    ...
    for inputs, labels in test_dataloader:  # use different dataloder in different phase
        ...
        soft_labels = torch.eye(len(class_names)).to(labels.device)[labels, :]
        ...
        for cls_idx in range(len(class_names)):
            ...
            tp = np.dot(
                (labels.cpu().data == cls_idx).numpy().astype(int),
        ...
        if index % check_minibatch == check_minibatch - 1:
            ...
            epoch_idx = "test"
            logger.info(
                "Epoch: {}, minibatch: {}, time used: {}".format(
                    epoch_idx, check_index, model_time
                )
            )
            logger.info(
    ...
    logger.info(
        "Epoch: {}, loss: {:.4f}, acc: {:.4f}".format(phase, epoch_loss, epoch_acc)
    )
    ...
    for cls_idx in range(len(class_names)):
        ...
        logger.info(
            "{} precision: {:.4f}, recall: {:.4f}, sensitivity: {:.4f}, specificity: {:.4f}".format(
                class_names[cls_idx], precision, recall, TPR, TNR
            )
        )
        ...
        if precision + recall != 0:
            ...
            print("{} F1: {:.4f}".format(class_names[cls_idx], f1))
            sum_f1 += f1

    logger.info("macro F1: {:.4f}".format(sum_f1 / len(class_names)))
    ...
    json.dump(
        json_log,
        open(os.path.join(draw_path, test_model_idx + "_log.json"), "w"),
        ensure_ascii=False,
        indent=2,
    )
...
def main(args):
    ...
    logger = get_logger(
        "Test_{}_{}".format(args.model_idx, args.dataroot.split("/")[-1])
    )
    ...
    test_datasets = torchvision.datasets.ImageFolder(
        test_dataroot, data_transforms["val"]
    )
    test_dataset_size = len(test_datasets)
    # skip minibatch none to draw 20 figs
    check_minibatch = (
        args.check_minibatch
        if args.check_minibatch is not None
        else test_dataset_size // (20 * batch_size)
    )

    test_dataloader = torch.utils.data.DataLoader(
        test_datasets, batch_size=batch_size, shuffle=False, num_workers=1
    )
    ...
    if num_classes == 0:
        ...
        if len(class_names) == num_classes:
            ...
            print(
                "classfication number of the model mismatch the dataset requirement of:",
                len(class_names),
            )
            return -1
    ...
    # get model
    pretrained_backbone = (
        False  # model is trained already, pretrained backbone weight is useless here
    )
...
def get_args_parser():
    ...
    # Model Name or index
    parser.add_argument(
        "--model_idx", default="ViT", type=str, help="Model Name or index"
    )

    # drop_rate, attn_drop_rate, drop_path_rate
    parser.add_argument(
        "--drop_rate", default=0.0, type=float, help="dropout rate , default 0.0"
    )
    parser.add_argument(
        "--attn_drop_rate",
        default=0.0,
        type=float,
        help="dropout rate Aftter Attention, default 0.0",
    )
    parser.add_argument(
        "--drop_path_rate",
        default=0.0,
        type=float,
        help="drop path for stochastic depth, default 0.0",
    )

    # Abalation Studies for MSHT
    parser.add_argument(
        "--cls_token_off", action="store_true", help="use cls_token in model structure"
    )
    parser.add_argument(
        "--pos_embedding_off",
        action="store_true",
        help="use pos_embedding in model structure",
    )
    # 'SimAM', 'CBAM', 'SE' 'None'
    parser.add_argument(
        "--att_module",
        default="SimAM",
        type=str,
        help="use which att_module in model structure",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="path to a model state-dict",
    )

    # Help tool parameters
    parser.add_argument(
        "--paint", action="store_false", help="paint in front desk"
    )
    parser.add_argument(
        "--enable_tensorboard",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="check and save attention map",
    )
    parser.add_argument(
        "--enable_visualize_check", action="store_true", help="check and save pics"
    )

    parser.add_argument(
        "--data_augmentation_mode", default=0, type=int, help="data_augmentation_mode"
    )
    ...
    # Prompt_Token_num
    parser.add_argument(
        "--Prompt_Token_num", default=20, type=int, help="Prompt_Token_num"
    )
    # PromptUnFreeze
    parser.add_argument(
        "--PromptUnFreeze",
        action="store_true",
        help="prompt tuning will all parameaters un-freezed",
    )
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="classification number, default 0 for auto-fit",
    )
    parser.add_argument(
        "--edge_size", default=224, type=int, help="edge size of input image"
    )  # 224 256 384 1000

    # Test setting parameters
    parser.add_argument(
        "--batch_size", default=1, type=int, help="testing batch_size default 1"
    )
    # check_minibatch for painting pics
    parser.add_argument(
        "--check_minibatch", default=None, type=int, help="check batch_size"
    )
