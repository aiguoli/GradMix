...
def adjust_learning_rate(optimizer, epoch, args):
    if epoch < args.warmup_epochs:  # for warmup
        lr = (
            args.lr * epoch / args.warmup_epochs
        )  # lr increase from zero to the setted lr
...
class patch_scheduler:
    ...
    def __init__(
        ...
        strategy=None,
        threshold=4.0,
        reducing_factor=0.933,
        fix_patch_size=None,
        patch_size_jump=None,
    ):
        super().__init__()
        ...
        if self.strategy in [
            ...
            "loss_back",
            "loss_hold",
        ]:  # start from big(easy) to samll(complex)
    ...
    def __call__(self, epoch, loss=0.0):
        ...
        if (
            self.strategy == "linear" or self.strategy == "reverse"
        ):  # reverse from big size to small
            if epoch < self.warmup_epochs:  # warmup
                puzzle_patch_size = 32  # fixed size for warmup
            else:
                puzzle_patch_size = self.patch_list[
                    min(
                        int(
            ...
            if epoch < self.warmup_epochs:
                puzzle_patch_size = (
                    32  # in warm up epoches, fixed patch size at 32 fixme exploring
                )
            else:
                group_idx = (epoch - self.warmup_epochs) % (
                    len(self.patch_list) * group_size
                )
...
class ratio_scheduler:
    ...
    def __init__(
        ...
        threshold=4.0,
        loss_reducing_factor=0.933,
        ratio_floor_factor=0.5,
        upper_limit=0.9,
        lower_limit=0.2,
    ):
    ...
    def __call__(self, epoch, loss=0.0):
        if self.strategy == "ratio-decay" or self.strategy == "decay":
            if epoch < self.warmup_epochs:  # for warmup
                ...
                max_ratio = min(
                    3 * self.basic_ratio, self.upper_limit
                )  # upper-limit of 0.9
                min_ratio = max(
                    self.basic_ratio * self.ratio_floor_factor, self.lower_limit
                )

                fix_position_ratio = min(
                    max(
                        (
                            (self.total_epoches - self.warmup_epochs)
                            - (epoch - self.warmup_epochs)
