...
def no_cls_token_s12_transform(
    tensor, height=12, width=12
):  # based on pytorch_grad_cam
...
def check_SAA(
    ...
    grad_cam = choose_cam_by_model(
        model, model_idx, edge_size, model_type=model_type
    )  # choose model
    ...
    for j in range(num_images):
        for type in checking_type:
            ...
            if type == "ori":
                ...
                if (
                    unknown_GT and not len(labels) == 1
                ):  # Ground Truth of the K+1 soft label
                ...
                if type == "tar":
                    ...
                    ax.set_title(
                        "{:.1%} {}".format(
                            softmax(outputs[j])[int(type)], class_names[int(type)]
                        )
                    )
                    # focus on the specific target class to create grayscale_cam
                    # grayscale_cam is generate on batch
                ...
                cam_img = show_cam_on_image(
                    inputs[j].cpu().numpy().transpose((1, 2, 0)),
                    grayscale_cam[j],
                    use_rgb=True,
                )  # Fixme: use rgb format input (already fixed)

                plt.imshow(cam_img)
                plt.pause(0.001)  # pause a bit so that plots are updated

            if images_so_far == num_images * len(
                checking_type
            ):  # complete when the pics is enough
                ...
                if (
                    writer is not None
                ):  # attach the pic to the tensorboard backend if avilable
...
def visualize_check(
    ...
    with torch.no_grad():
        ...
        for j in range(num_images):  # each batch input idx: j
            ...
            ax.set_title(
                "Pred: {}\n True: {}".format(
                    class_names[preds[j]], class_names[int(labels[j])]
                )
            )
            imshow(inputs.cpu().data[j])
